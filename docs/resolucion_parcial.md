...
- **Implementación de Reportes (HU-09)**: Se implementó la opción de "Productos por categoría" en el Menú. El sistema lista las categorías habilitadas, solicita un ID y hace uso de `productoRepo.buscarPorCategoria(id)` para imprimir la información filtrada por JPQL (ID, nombre, precio y stock). Si una categoría carece de productos activos, se muestra un mensaje informativo explícito.
